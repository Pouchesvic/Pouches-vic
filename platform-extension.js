...
async function createPublicPlatformOrder(body) {
  ...
  const trusted = { ...body, manual_location:!text(body.address) };
  delete trusted.delivery_fee;delete trusted.delivery_fee_cents;delete trusted.zone_id;delete trusted.lat;delete trusted.lng;delete trusted.address_lat;delete trusted.address_lng;
...
function businessOrderNotificationHtml(order) {
  ...
  return `<!doctype html><html><body style="font-family:Arial,sans-serif;color:#171717"><div style="max-width:640px;margin:auto"><h1>New order #${escapeHtml(order.order_no)}</h1><p>${escapeHtml(order.created_at)}</p>${flags}<p><b>Requested delivery:</b> ${escapeHtml(requestedDate||'Not recorded')}${order.requested_window_label?` • ${escapeHtml(order.requested_window_label)}`:''}</p><h2>Customer</h2><p style="font-size:18px"><b>${escapeHtml(order.customer_name||'Name not supplied')}</b><br>${escapeHtml(order.customer_phone||'')}${order.address?`<br>${escapeHtml(order.address)}`:''}${order.meeting_instructions?`<br>${escapeHtml(order.meeting_instructions)}`:''}${order.customer_email?`<br>${escapeHtml(order.customer_email)}`:''}</p>${order.delivery_notes?`<p><b>Delivery Notes</b><br>${escapeHtml(order.delivery_notes)}</p>`:''}<table style="width:100%;border-collapse:collapse">${itemRows}<tr><td style="padding-top:12px">Products</td><td style="padding-top:12px;text-align:right">${money(order.subtotal_cents)}</td></tr><tr><td>Delivery</td><td style="text-align:right">$0.00</td></tr><tr><td style="font-size:18px;font-weight:bold;padding-top:9px">TOTAL</td><td style="font-size:18px;font-weight:bold;padding-top:9px;text-align:right">${money(order.total_cents)}</td></tr></table><p><b>Payment:</b> ${escapeHtml(order.payment_method||'Not specified')}${order.payment_note?`<br>${escapeHtml(order.payment_note)}`:''}</p><p><a href="${PUBLIC_BASE_URL}/admin">Open Control Room</a></p></div></body></html>`;
}
...
function platformSettlementEntries(orders){
  ...
  for(const o of orders){
    const snapshot=all('SELECT * FROM settlement_entries WHERE order_id=? ORDER BY created_at,id',o.id);
    if(snapshot.length){entries.push(...snapshot);continue;}
...
function settlementPeriodReport(period) {
  ...
  return {period:{...period},driver,closed:period.status==='closed',started_at:period.started_at,ended_at:end,sold_website:webQty,sold_off_website:offsiteOrderQty+manualOffsite,taken_for_self:selfQty,personal_use_charge_cents:personalUseCharge,other_adjustments:otherQty,accountable_items:accountable,boss_rate_cents:bossRate,boss_share_before_credits_cents:bossShare,boss_credit_cents:bossCredit,calculated_net_cents:calculatedNet,adjustment_cents:adjustment,adjustment_reason:text(period.adjustment_reason),send_to_boss_cents:sendToBoss,boss_owes_driver_cents:bossOwesDriver,company_owes_driver_cents:bossOwesDriver,final_direction:netBossDue>=0?'send_to_company':'company_owes_driver',final_amount_cents:Math.abs(netBossDue),cash_in_driver_hands_cents:cashInHand,driver_keeps_cents:driverKeeps,owes_other_drivers_cents:owesDrivers,receives_from_drivers_cents:receivesDrivers,delivery_fees_cents:deliveryFees,tips_cents:tips,driver_tips_cents:driverTips,starting_inventory:starting,expected_ending_inventory:expected,actual_ending_inventory:actual,variance:actual==null||expected==null?null:actual-expected,orders:orders.map(o=>({id:o.id,order_no:o.order_no,source:o.source,qty:platformOrderQty(o.id),completed_at:o.completed_at,total_cents:o.total_cents,delivery_fee_cents:o.delivery_fee_cents,tip_cents:o.tip_cents,company_rate_cents_snapshot:o.company_rate_cents_snapshot,driver2_to_driver1_rate_cents_snapshot:o.driver2_to_driver1_rate_cents_snapshot})),payments:pays,transactions:tx,entries};
}
