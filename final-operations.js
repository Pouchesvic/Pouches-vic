...
module.exports = function createFinalOperations({ db, companyStock, now, id, text, int, bool, jsonText, safeJson, addOrderEvent }) {
  ...
  function promotionalCan(driver,orderId,body){
    const order=one('SELECT * FROM orders WHERE id=?',orderId);if(!order||!canFulfillOrder(driver.id,order))throw new Error('Order not found.');if(one('SELECT 1 FROM order_promotional_free_cans WHERE order_id=?',orderId))throw new Error('This customer transaction already has its one promotional free can.');const productId=text(body.product_id);if(!productId)throw new Error('Choose a product.');
