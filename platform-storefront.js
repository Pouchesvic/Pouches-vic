(() => {
  ...
  async function selectAddressFeature(feature,input,statusId){
    ...
    const lat=Number(center[1]),lng=Number(center[0]);
    if(!Number.isFinite(lat)||!Number.isFinite(lng)){deliveryState={slug:window.currentSlug||'victoria',address,lat:null,lng:null,quote:null,method:deliveryState.method||'delivery'};renderAddressStatus(statusId,null);return;}
    deliveryState={slug:window.currentSlug||'victoria',address,lat,lng,quote:null,method:deliveryState.method||'delivery'};
    const quote=await getQuote({address,lat,lng}); deliveryState.quote=quote;saveAddressState();renderAddressStatus(statusId,quote);
  }
  ...
  function showConfirmedZone(zone,q){
    let card=document.getElementById('pvZoneConfirmed');if(!card){card=document.createElement('div');card.id='pvZoneConfirmed';card.className='pv-zone-confirmed';document.getElementById('customerAddress')?.closest('.field')?.insertAdjacentElement('afterend',card);}card.innerHTML=`<b>DELIVERY AREA CONFIRMED</b>${E(q.zone.name)} • ${Number(q.zone.fee_cents)===0?'FREE':money(q.zone.fee_cents)}${q.override?.applied?' • your saved delivery exception is applied':''}`;
  }
  ...
  async function boot(){
    ...
    const timer=setInterval(async()=>{
      ...
      const slug=window.currentSlug||'victoria';if(slug!==watchedSlug){watchedSlug=slug;sessionStorage.removeItem('pv_delivery_address_'+slug);await onTerritoryReady(slug);}
    },650);
