(() => {
  ...
  async function renderCompanyInventory() {
    ...
    try {
      ...
      host.innerHTML=`<div class="row"><div><div class="cs-title">Stock</div><div class="muted">Local ${E(territory.name)}</div></div><button id="csStockHistory" class="btn ghost">HISTORY</button></div>${networkSummaryHtml()}<div class="three"><button id="csStockAdd" class="btn">ADD STOCK</button><button id="csNetworkMove" class="btn ghost">MOVE STOCK</button><button id="csNetworkFix" class="btn ghost">FIX STOCK</button></div><div id="csInventoryBody"></div>`;
      renderSimpleStock(territory.id);q('csStockHistory').onclick=renderHistoryView;q('csStockAdd').onclick=()=>openSimpleReceive(territory.id);q('csNetworkMove').onclick=openNetworkMove;q('csNetworkFix').onclick=openNetworkFix;
  ...
  function openSimpleReceive(territoryId){
    const locations=networkLocationOptions(),local=state.network?.locals?.find(x=>x.territory.id===territoryId),preferred=local?.main_driver_id?`driver:${local.territory.id}|${local.main_driver_id}`:'reserve';
    modal(`<h2>Add Stock</h2><div class="good">Record where the cans physically are right now. Stock added to a driver becomes that driver's live storefront stock. Company Reserve stays off the storefront until moved.</div><div class="field"><label>PRODUCT</label><select id="csSimpleProduct">${productOptions()}</select></div><div class="field"><label>HOW MANY CANS ARRIVED?</label><input id="csSimpleQty" type="number" min="1"></div><div class="field"><label>WHERE ARE THEY NOW?</label><select id="csSimpleWhere">${locations}</select></div><div class="field"><label>NOTE <span class="muted">(optional)</span></label><input id="csSimpleNote" placeholder="Shipment received"></div><button id="csSimpleSave" class="btn" style="width:100%">ADD STOCK</button>`);
    if([...q('csSimpleWhere').options].some(x=>x.value===preferred))q('csSimpleWhere').value=preferred;
    q('csSimpleSave').onclick=async()=>{const loc=parseNetworkLocation(q('csSimpleWhere').value),qty=Math.max(0,Math.trunc(Number(q('csSimpleQty').value)||0)),productId=q('csSimpleProduct').value,note=q('csSimpleNote').value.trim()||'Shipment received';if(!qty)return alert('Enter how many cans arrived.');try{if(loc.kind==='company_reserve')await request('/api/admin/company-stock/receive',{method:'POST',body:JSON.stringify({product_id:productId,target:'company_reserve',qty,note})});else await request('/api/admin/driver-inventory/adjust',{method:'POST',body:JSON.stringify({driver_id:loc.driver_id,territory_id:loc.territory_id,product_id:productId,qty_delta:qty,kind:'stock_received',note})});closeModal();state.data=null;state.network=null;await renderCompanyInventory();}catch(error){alert(error.message)}};
  }
  ...
  function networkLocationOptions(){
    const reserve='<option value="reserve">Prince George — Company Reserve</option>';
    return reserve+networkDrivers().map(x=>`<option value="driver:${E(x.territory.id)}|${E(x.driver.id)}">Local ${E(x.territory.name)} — ${E(x.driver.name)}</option>`).join('');
  }
  function parseNetworkLocation(value){
    if(value==='reserve')return {kind:'company_reserve',territory_id:'',driver_id:''};
    const [territory_id,driver_id]=String(value||'').replace(/^driver:/,'').split('|');
    return {kind:'driver',territory_id,driver_id};
  }
  function networkProductsFor(value,includeZero=false){
    if(value==='reserve'){
  ...
  function refreshNetworkMoveProducts(){
    const products=networkProductsFor(q('csMoveFrom')?.value,false);q('csMoveProduct').innerHTML=products.map(x=>`<option value="${E(x.product_id)}">${E(x.brand)} ${E(x.flavor)} — ${x.qty} available</option>`).join('');
  }
  async function saveNetworkMove(){
    try{const from=parseNetworkLocation(q('csMoveFrom').value),to=parseNetworkLocation(q('csMoveTo').value);if(q('csMoveFrom').value===q('csMoveTo').value)return alert('Choose a different destination.');await request('/api/admin/inventory-network/transfer',{method:'POST',body:JSON.stringify({from_kind:from.kind,from_driver_id:from.driver_id,from_territory_id:from.territory_id,to_kind:to.kind,to_driver_id:to.driver_id,to_territory_id:to.territory_id,product_id:q('csMoveProduct').value,qty:q('csMoveQty').value,note:q('csMoveNote').value})});closeModal();state.data=null;state.network=null;await renderCompanyInventory();}catch(e){alert(e.message)}
  }
  function openNetworkFix(){
    modal(`<h2>Fix Stock</h2><div class="good">For a physical recount, lost can, stolen can, or stock that was found. Every correction stays in inventory history.</div><div class="field"><label>WHERE</label><select id="csFixDriver">${networkLocationOptions()}</select></div><div class="field"><label>PRODUCT</label><select id="csFixProduct"></select></div><div class="field"><label>WHAT HAPPENED?</label><select id="csFixKind"><option value="found">FOUND / ADD</option><option value="lost">LOST / REMOVE</option><option value="stolen">STOLEN / REMOVE</option><option value="manual_correction">OTHER CORRECTION / REMOVE</option></select></div><div class="field"><label>QUANTITY</label><input id="csFixQty" type="number" min="1" value="1"></div><div class="field"><label>REASON / NOTE</label><input id="csFixNote" placeholder="Required"></div><button class="btn" id="csFixSave" style="width:100%">UPDATE STOCK</button>`);
    q('csFixDriver').onchange=refreshNetworkFixProducts;q('csFixKind').onchange=refreshNetworkFixProducts;refreshNetworkFixProducts();q('csFixSave').onclick=saveNetworkFix;
  }
  function refreshNetworkFixProducts(){
    const includeZero=q('csFixKind')?.value==='found',products=networkProductsFor(q('csFixDriver')?.value,includeZero);q('csFixProduct').innerHTML=products.map(x=>`<option value="${E(x.product_id)}">${E(x.brand)} ${E(x.flavor)} — ${x.qty} available</option>`).join('');
  }
  async function saveNetworkFix(){
    try{const loc=parseNetworkLocation(q('csFixDriver').value),kind=q('csFixKind').value,qty=Math.max(1,Math.trunc(Number(q('csFixQty').value)||1)),delta=kind==='found'?qty:-qty,body={product_id:q('csFixProduct').value,qty_delta:delta,kind:'stock_'+kind,note:q('csFixNote').value};if(loc.kind==='company_reserve')await request('/api/admin/company-reserve/adjust',{method:'POST',body:JSON.stringify(body)});else await request('/api/admin/driver-inventory/adjust',{method:'POST',body:JSON.stringify({...body,driver_id:loc.driver_id,territory_id:loc.territory_id})});closeModal();state.data=null;state.network=null;await renderCompanyInventory();}catch(e){alert(e.message)}
  }
